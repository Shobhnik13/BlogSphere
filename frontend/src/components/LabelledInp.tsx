...
const LabelledInp = ({label,placeholder,onChange,type}:labelledInpProps) => {
  return (
    <div>
        ...
        <input type={type}  id= {label} className="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg block w-full p-3" placeholder={placeholder} required  onChange={onChange}/>
    </div>
  )
}
